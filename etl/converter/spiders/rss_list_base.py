...
class RSSListBase(RSSBase, LomBase):
    ...
    def getCSVValue(self, response, fieldName):
        data = list(map(lambda x: x.strip(),response.meta['row'][self.mappings[fieldName]].split(";")))
    ...
    def getLOMGeneral(self, response):
        ...
        general.replace_value('language', self.getCSVValue(response, CSVBase.COLUMN_LANGUAGE))
        general.replace_value('keyword', self.getCSVValue(response, CSVBase.COLUMN_KEYWORD))
        return general
