...
class DigitallearninglabSpider(scrapy.Spider, LrmiBase):
  ...
  def getLOMGeneral(self, response):
    ...
    general.add_value('description', HTMLParser().unescape(response.meta['item'].get('teaser')))
  ...
  def getValuespaces(self, response):
    ...
    try:
      range = response.xpath('//ul[@class="sidebar__information"]/li[@class="sidebar__information-item"]/*[contains(@class,"icon-level")]/parent::*//text()').get().replace('Stufe', '').strip().split(' - ')
      if len(range):
        valuespaces.add_value('educationalContext', ValuespaceHelper.educationalContextByGrade(range))
    except:
      pass
    try:
      discipline = response.xpath('//ul[@class="sidebar__information"]/li[@class="sidebar__information-item"]/*[contains(@class,"icon-subject")]/parent::*//text()').getall()
    ...
    try:
      toolType = list(map(lambda x: x.strip(), response.xpath('//ul[@class="sidebar__information"]/li[@class="sidebar__information-item"]/*[contains(@class,"icon-settings")]/parent::*//text()').getall()))
