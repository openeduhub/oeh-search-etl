...
class OEHRSSSpider(RSSListBase):
    ...
    def getBase(self, response):
        ...
        base.replace_value('origin', self.getCSVValue(response, CSVBase.COLUMN_SOURCE_TITLE))
        return base
    def getLOMLifecycle(self, response = None) -> LomLifecycleItemloader:
      lifecycle = RSSListBase.getLOMLifecycle(self, response)
      lifecycle.add_value('role', 'author')
      lifecycle.add_value('organization', self.getCSVValue(response, CSVBase.COLUMN_SOURCE_TITLE))
      lifecycle.add_value('url', self.getCSVValue(response, CSVBase.COLUMN_SOURCE_URL))
      return lifecycle